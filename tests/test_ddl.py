from sqlalchemy import Column, ForeignKey, event, func, inspect, select, text
...
class DDLTestCase(BaseTestCase):
    ...
    def test_create_table_nested_types(self):
        ...
        self.assertEqual(
            self.compile(CreateTable(table)),
            "CREATE TABLE t1 " "(x Int32, y Array(Array(String))) " "ENGINE = Memory",
        )
    ...
    def test_create_table_nullable_nested_nullable(self):
        table = Table(
            ...
            self.metadata(),
            Column("x", types.Int32, primary_key=True),
            Column("y", types.Nullable(types.Array(types.Nullable(types.String)))),
            engines.Memory(),
        )
    ...
    def test_add_column(self):
        col = Column("x2", types.Int8, nullable=True, clickhouse_after=text("x1"))
    ...
    def test_create_simple_aggregate_function(self):
        table = Table(
            "t1",
            self.metadata(),
            Column("total", types.SimpleAggregateFunction(func.sum(), types.UInt32)),
            engines.Memory(),
        )
    ...
    def test_drop_table_clause(self):
        table = Table("t1", self.metadata(), Column("x", types.Int32, primary_key=True))

        self.assertEqual(self.compile(DropTable(table)), "DROP TABLE t1")
        self.assertEqual(
            self.compile(DropTable(table, if_exists=True)), "DROP TABLE IF EXISTS t1"
        )
    ...
    def test_table_drop_on_cluster(self):
        ...
        self.assertEqual(self.compile(DropTable(table, if_exists=True)), drop_sql)
        ...
        class Statistics(Base):
            ...
            __table_args__ = (
                engines.CollapsingMergeTree(
                    sign, partition_by=func.toYYYYMM(date), order_by=(date, grouping)
                ),
            )
