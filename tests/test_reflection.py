...
class ReflectionTestCase(BaseTestCase):
    def test_server_default(self):
        ...
        with self.create_table(table):
            self.assertEqual(
                [c["default"] for c in inspect(self.session.bind).get_columns("t")],
                [None, "''"],
            )
    ...
    def test_array_of_array(self):
        coltype = self._type_round_trip(types.Array(types.Array(types.Int32)))[0][
            "type"
        ]
    ...
    def test_not_null(self):
        metadata = self.metadata()
        table = Table(
            "t", metadata, Column("x", types.Int32, nullable=False), engines.Memory()
        )
        with self.create_table(table):
    ...
    def test_low_cardinality(self):
        coltype = self._type_round_trip(types.LowCardinality(types.String))[0]["type"]
    ...
    def test_map(self):
        coltype = self._type_round_trip(types.Map(types.String, types.String))[0][
            "type"
        ]
    ...
    def test_enum8(self):
        ...
        self.assertEqual({o.name: o.value for o in coltype.enum_class}, enum_options)
    ...
    def test_datetime64(self):
        coltype = self._type_round_trip(types.DateTime64(2, "Europe/Moscow"))[0]["type"]
    ...
    def test_datetime(self):
        coltype = self._type_round_trip(types.DateTime("Europe/Moscow"))[0]["type"]
    ...
    def test_aggregate_function(self):
        ...
        coltype = self._type_round_trip(
            types.AggregateFunction(func.argMin(), types.Float32, types.Float32)
        )[0]["type"]
        self.assertIsInstance(coltype, types.AggregateFunction)
