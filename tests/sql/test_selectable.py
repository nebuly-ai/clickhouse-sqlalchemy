...
from clickhouse_sqlalchemy import Table, engines, select, types
...
class SelectTestCase(BaseTestCase):
    def _make_table(self, table_name=None, *columns):
        ...
        columns = (columns or (Column("x", types.Int32, primary_key=True),)) + (
            engines.Memory(),
        )
    ...
    def test_group_by_with_modifiers(self):
        ...
        self.assertEqual(self.compile(query), "SELECT t1.x FROM t1 GROUP BY t1.x")
        ...
        self.assertEqual(
            self.compile(query), "SELECT t1.x FROM t1 GROUP BY t1.x WITH ROLLUP"
        )

        query = select(table.c.x).group_by(table.c.x).with_totals()
        self.assertEqual(
            self.compile(query), "SELECT t1.x FROM t1 GROUP BY t1.x WITH TOTALS"
        )
        ...
        query = select(table.c.x).group_by(table.c.x).with_rollup().with_totals()
        self.assertEqual(
            self.compile(query),
    ...
    def test_sample(self):
        ...
        self.assertEqual(
            self.compile(query), "SELECT t1.x FROM t1 SAMPLE %(param_1)s GROUP BY t1.x"
        )
    ...
    def test_final(self):
        ...
        self.assertEqual(self.compile(query), "SELECT t1.x FROM t1 FINAL GROUP BY t1.x")

    def test_limit_by(self):
        table = self._make_table()

        query = select(table.c.x).order_by(table.c.x).limit_by([table.c.x], limit=1)
        self.assertEqual(
            self.compile(query),
    ...
    def test_nested_type(self):
        ...
        query = select(table.c.parent.child1).where(
            and_(
                table.c.parent.child1 == [1, 2], table.c.parent.child2 == ["foo", "bar"]
            )
        )
        self.assertEqual(
            self.compile(query, literal_binds=True),
    ...
    def test_join(self):
        ...
        def make_statement(
            type=None, strictness=None, distribution=None, full=False, isouter=False
        ):
            join = table_1.join(
                table_2,
        ...
        for make_statement in (make_statement, make_statement_select_join):
            ...
            self.assertEqual(
                self.compile(
                    make_statement(
                        type="LEFT OUTER", distribution="global", strictness="all"
                    )
                ),
                "SELECT table_1.x FROM table_1 "
            ...
            self.assertEqual(
                self.compile(
                    make_statement(
                        type="RIGHT OUTER", distribution="global", strictness="all"
                    )
                ),
                "SELECT table_1.x FROM table_1 "
            ...
            self.assertEqual(
                self.compile(make_statement(isouter=False, full=False, type="INNER")),
                "SELECT table_1.x FROM table_1 "
                "INNER JOIN table_2 ON table_2.y = table_1.x",
            )
            ...
            self.assertEqual(
                self.compile(
                    make_statement(isouter=False, full=False, type="LEFT OUTER")
                ),
                "SELECT table_1.x FROM table_1 "
                "LEFT OUTER JOIN table_2 ON table_2.y = table_1.x",
            )
            ...
            self.assertEqual(
                self.compile(
                    make_statement(
                        isouter=False, full=False, type="LEFT", strictness="ALL"
                    )
                ),
                "SELECT table_1.x FROM table_1 "
            ...
            self.assertEqual(
                self.compile(
                    make_statement(
                        isouter=False, full=False, type="LEFT", strictness="ANY"
                    )
                ),
                "SELECT table_1.x FROM table_1 "
    ...
    def test_sql_expression_join(self):
        ...
        join = expression.join(
            table_1, table_2, onclause=table_1.c.x == table_2.c.x, isouter=False
        )

        self.assertEqual(
            self.compile(join), "table_1 INNER JOIN table_2 ON table_1.x = table_2.x"
        )
    ...
    def test_distinct_on(self):
        ...
        self.assertEqual(self.compile(query), "SELECT DISTINCT ON (t1.x) t1.x FROM t1")
    ...
    def test_map_type(self):
        ...
        query = select(func.mapKeys(table.c.y), func.mapValues(table.c.y)).where(
