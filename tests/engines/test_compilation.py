...
from clickhouse_sqlalchemy import Table, engines, get_declarative_base, types
...
class AggregatingMergeTree(EngineTestCaseBase):
    def test_basic(self):
        class TestTable(self.base):
            ...
            __table_args__ = (
                engines.AggregatingMergeTree(partition_by=date, order_by=(date, x)),
            )
        ...
        class TestTable(self.base):
            ...
            __table_args__ = (
                engines.ReplicatedAggregatingMergeTree(
                    "/table/path", "name", partition_by=date, order_by=(date, x)
                ),
            )
        ...
        class TestTable(self.base):
            ...
            __table_args__ = (
                engines.ReplacingMergeTree(
                    version="version", partition_by="date", order_by=("date", "x")
                ),
            )
...
class MiscEnginesTestCase(EngineTestCaseBase):
    ...
    def test_distributed(self):
        class TestTable(self.base):
            ...
            __table_args__ = (
                engines.Distributed("cluster", "test", "merge_distributed1", "rand()"),
            )
