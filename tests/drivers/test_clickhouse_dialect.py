...
from clickhouse_sqlalchemy import Table, engines, make_session, types
...
from tests.testcase import BaseAsynchTestCase, BaseTestCase
from tests.util import require_server_version, run_async, with_native_and_http_sessions
...
class ClickHouseDialectTestCase(BaseTestCase):
    ...
    def test_has_table_with_schema(self):
        self.assertFalse(self.dialect.has_table(self.session, "bad", schema="system"))
        self.assertTrue(
    ...
    def test_empty_set_expr(self):
        ...
        rv = self.session.query(numbers.c.number).filter(numbers.c.number.in_([])).all()
...
class ClickHouseAsynchDialectTestCase(BaseAsynchTestCase):
    ...
    async def test_has_table(self):
        self.assertFalse(await self.run_inspector_method("has_table", self.table.name))

        await self.run_sync(self.test_metadata.create_all)

        self.assertTrue(await self.run_inspector_method("has_table", self.table.name))

    async def test_has_table_with_schema(self):
        self.assertFalse(
            await self.run_inspector_method("has_table", "bad", schema="system")
        )
        self.assertTrue(
            await self.run_inspector_method("has_table", "columns", schema="system")
        )
    ...
    async def test_get_table_names_with_schema(self):
        ...
        db_tables = await self.run_inspector_method("get_table_names", schema="system")
    ...
    async def test_get_view_names_with_schema(self):
        ...
        db_views = await self.run_inspector_method("get_view_names", test_database)
    ...
    async def test_empty_set_expr(self):
        ...
        def _run(session):
            return (
                session.query(numbers.c.number).filter(numbers.c.number.in_([])).all()
            )
...
class CachedServerVersionTestCase(BaseTestCase):
    ...
    def _test_server_version_any(self, uri):
        engine_session = make_session(
            create_engine(
                system_http_uri, connect_args=dict(server_version="123.45.67.89")
            )
        )
...
class CachedServerVersionAsyncTestCase(BaseAsynchTestCase):
    async def test_server_version_asynch(self):
        engine_session = make_session(
            create_async_engine(
                system_asynch_uri, connect_args=dict(server_version="123.45.67.89")
            ),
            is_async=True,
        )
