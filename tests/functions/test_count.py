...
from clickhouse_sqlalchemy import Table, types
from tests.testcase import CompilationTestCase


class CountTestCaseBase(CompilationTestCase):
    table = Table(
        "t1", CompilationTestCase.metadata(), Column("x", types.Int32, primary_key=True)
    )
    ...
    def test_count_distinct(self):
        ...
        self.assertEqual(
            self.compile(query), "SELECT count(distinct(t1.x)) AS count_1 FROM t1"
        )

    def test_count_no_column_specified(self):
        query = self.session.query(func.count()).select_from(self.table)
        self.assertEqual(self.compile(query), "SELECT count(*) AS count_1 FROM t1")
