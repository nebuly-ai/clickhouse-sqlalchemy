...
from sqlalchemy.sql import ClauseElement, compiler, expression
...
class ClickHouseDDLCompiler(compiler.DDLCompiler):
    def _get_default_string(self, default, name):
        if default == "":
            return "''"
        sa_util.assert_arg_type(default, (String, ClauseElement, TextClause), name)
    ...
    def visit_merge_tree(self, engine):
        ...
        param = "()" if param is None else self._compile_param(to_list(param))

        text = "{0}{1}\n".format(engine.name, param)
        if engine.partition_by:
            text += " PARTITION BY {0}\n".format(
                self._compile_param(
                    engine.partition_by.get_expressions_or_columns(), opt_list=True
                )
            )
        if engine.order_by:
            text += " ORDER BY {0}\n".format(
        ...
        if engine.primary_key:
            if not engine.order_by:
                text += " ORDER BY {0}\n".format(
                    self._compile_param(
                        engine.primary_key.get_expressions_or_columns(), opt_list=True
                    )
                )
        ...
        if engine.sample_by:
            text += " SAMPLE BY {0}\n".format(
                self._compile_param(engine.sample_by.get_expressions_or_columns()[0])
            )
        if engine.ttl:
    ...
    def visit_engine(self, engine):
        ...
        for param in engine_params:
            if isinstance(param, tuple):
                compiled = "(" + ", ".join(self._compile_param(p) for p in param) + ")"
            else:
    ...
    def visit_create_materialized_view(self, create):
        ...
        text += "AS " + self.sql_compiler.process(mv.mv_selectable, literal_binds=True)
